// Note: The code is a mess and need to be cleaned up :)
...
const optionDefinitions = [
    ...
    { name: 'update_password', type: Boolean, alias: 'u' }
]
...
if (options.config === undefined) {
    console.log(chalk.red("You need to provide a configuration file first! (--config file_name.json)"));
    process.exit(1);
}
const dConfig = require('./' + options.config);
...
getToken(defaultEmail, defaultPassword, function (response) {
    if (response.data.success) {
        ...
        if (options.update_password === true) {
